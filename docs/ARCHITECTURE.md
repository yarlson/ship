...
| ---------- | ------------------------------------------------------ | ------------------------------------------ |
| `cli`      | Parse `ship [-i key] [-p port] user@host image[:tag]`. | No I/O beyond flag parsing.                |
...
CLI Args → cli.Parse() → Config
  ...
  → workflow.Run(Config)
    → Stage 1: Tag → localhost:5001/<image>
    → Stage 2: Registry → ensure localhost:5001
    → Stage 3: Push → transfer tag available in local registry
    → Stage 4: Tunnel → SSH reverse tunnel (background process)
    → Stage 5: Pull+Restore → remote host has original image tag
...
- Keep the workflow narrow: transfer one image, do not deploy it.
