...
func Pull(ctx context.Context, cfg cli.Config, originals, transfers []string) error {
	...
	for i, original := range originals {
		...
		_, _, exitCode, err := ssh.RunRemoteCommand(ctx, cfg.KeyPath, cfg.Port, cfg.User, cfg.Host, pullCmd)
		...
		_, _, exitCode, err = ssh.RunRemoteCommand(ctx, cfg.KeyPath, cfg.Port, cfg.User, cfg.Host, tagCmd)
		if err != nil || exitCode != 0 {
